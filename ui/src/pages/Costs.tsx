...
const PRESET_I18N_KEYS: Record<DatePreset, string> = {
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs.selectCompanyMessage")} />;
  }
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} ${t("costs.pausedAgents").toLowerCase()} · ${budgetData?.pausedProjectCount ?? 0} ${t("costs.pausedProjects").toLowerCase()}`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}`
                    : t("costs.noMonthlyCapConfigured")
              }
              icon={Coins}
            />
            <MetricTile
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `${t("common.budget")} ${formatCents(spendData.summary.budgetCents)}`
              ...
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                ...
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  ...
                  <MetricTile
                    ...
                    subtitle={t("costs.budgetOverridesAwaiting")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.companyWidePolicy")
                            : scopeType === "agent"
                              ? t("costs.agentMonthlyPolicy")
                              : t("costs.projectLifetimePolicy")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
